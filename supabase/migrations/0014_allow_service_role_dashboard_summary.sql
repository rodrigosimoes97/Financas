-- Permitir execução segura de get_dashboard_summary em 2 modos:
-- 1) authenticated/anon com JWT de usuário -> exige auth.uid() = p_user_id
-- 2) service_role (server-side cache) -> permite sem auth.uid()

-- Remover possível overload legado para evitar conflitos de schema cache
...
DECLARE
  ...
  v_role_claim text := coalesce(current_setting('request.jwt.claim.role', true), '');
  v_is_service boolean := (v_role_claim = 'service_role');
...
GRANT EXECUTE ON FUNCTION public.get_dashboard_summary(uuid, date, date, date) TO service_role;
...
COMMENT ON FUNCTION public.get_dashboard_summary(uuid, date, date, date)
  IS 'Dashboard summary RPC with dual auth mode: user JWT or service_role.';

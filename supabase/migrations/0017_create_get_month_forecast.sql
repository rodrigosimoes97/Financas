-- Corrige definitivamente get_month_forecast com assinatura esperada
-- e ajusta funções dependentes para usar retorno tabular.

-- Diagnóstico (executar manualmente no SQL editor quando necessário):
...
-- Remove assinatura conflitante/legada
DROP FUNCTION IF EXISTS public.get_month_forecast(uuid, date, date, date);
...
-- Ajuste de dependência: get_financial_insights usava get_month_forecast como jsonb.
...
-- Ajuste de dependência: get_dashboard_summary passa a serializar linha da forecast table para jsonb
